    ...
    for depth in range(max_depth):
        ...
        try:
            async with httpx.AsyncClient(follow_redirects=True, timeout=12) as client:
                ...
                if media_links:
                    # Return highest quality or first media link found
            ...
            logger.warning("Crawl resolve error for %s: %s", current_url, e)
            break
...
async def resolve_stream(url: str, headers: dict) -> tuple[str, dict]:
    ...
    if "moviezwap" in url_lower or "moviezzwaphd" in url_lower:
        try:
            ...
            logger.warning("MoviezWap resolver error: %s", e)
    ...
    try:
        ...
        logger.warning("Universal resolver error: %s", e)
